function [sens,Col,Row,speciesNames] = sensAnalysis(perturb,dataset)
...
tspan = [0 5]; %this is how long you want the simulations to be run
tspan0 = [0 5]; %this is how long you want the simulations to be run
...
%all species
[t,y] = ode15s(@modelODE,tspan0,y0,options,params);  % this is an initial simulation using the modelODE, t = time, y = matrix with activity of each species at that time
...
%     tspan = [0 700]; 
    options = []; 
    [t2,y2] = ode15s(@modelODE,tspan,y0,options,params); % a new simulation with the new parameter value
...
% % xf=get(gca,'position');
% set(h,'FontSize',16);
% saveas(gcf,['Sensitivity Matrix (M' num2str(dataset) ' Stimulation)'],'tiffn');
% close
%  
%%
...
% % xf=get(gca,'position');
% set(h,'FontSize',16);
% saveas(gcf,['Sensitivity Matrix (M' num2str(dataset) ' Stimulation Dim)'],'tiffn');
% close
% 
end